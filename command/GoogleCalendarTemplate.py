...
def YoutubePush(title, scheduled_start_time_tokyo, youtube_url):
    google_calendar = GoogleCalendar(
        title, scheduled_start_time_tokyo
    )  # initを実行するために必要
    Calendar_id = os.environ["CALENDAR_ID"]
...
class GoogleCalendar:
    def __init__(self, title, scheduled_start_time_tokyo):
        ...
        ):  # creds.validはTrueかFalseを返し、切れている場合はFalse
            if (
    ...
    def Add_Youtube(self, Calendar_id, youtube_url):
        ...
        event = {
            ...
            "start": {"dateTime": scheduled_start_time, "timeZone": "Asia/Tokyo"},
            "end": {"dateTime": scheduled_start_time, "timeZone": "Asia/Tokyo"},
            "reminders": {
                "useDefault": False,
                "overrides": [
