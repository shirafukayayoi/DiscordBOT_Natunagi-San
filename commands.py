...
from command.GoogleCalendarTemplate import CalendarPush, YoutubePush
from command.Novel_Sale_List import get_manga_data, get_novel_data
...
def setup(bot: commands.Bot):
    ...
    async def setrss(interaction: discord.Interaction, url: str):
        if bot.add_rss_url(url):
            ...
            await interaction.response.send_message(
                f"このRSS URLは既に追加されています。"
            )

    @bot.tree.command(
        name="rss-list", description="登録されているRSSフィードのURLを表示します"
    )
    async def listrss(interaction: discord.Interaction):
        embed = discord.Embed(
            title="登録されているRSSフィードのURLリスト",
    ...
    @bot.tree.command(
        name="rss-remove", description="登録されているRSSフィードのURLを削除します"
    )
    ...
    @bot.tree.command(
        name="send-minute",
        description="指定した分数ごとにメッセージを送信するための時間を設定します",
    )
    ...
    async def summon_playlist(interaction: discord.Interaction, playlist: str):
        if playlist == "とにかく詰め込め！":
            await interaction.response.send_message(
                "https://youtube.com/playlist?list=PLHh1qLt2rZLK4QFgf_W0pZLAMSiipfT3o&si=pf23FQmirl2agzLt"
            )
        elif playlist == "ブルーアーカイブOST":
            await interaction.response.send_message(
                "https://youtube.com/playlist?list=PLh6Ws4Fpphfqr7VL72Q6HK5Ole9YI54hv&si=yL9IWx3zvuuw3YcP"
            )

    @bot.tree.command(
        name="youtube-set-rss", description="YouTubeのRSSフィードのURLを追加します"
    )
    async def youtube_setrss(interaction: discord.Interaction, name: str, url: str):
        if bot.add_youtube_rss_url(name, url):
            ...
            await interaction.response.send_message(
                f"このYouTube RSS URLは既に追加されています。"
            )

    @bot.tree.command(
        name="youtube-list-rss",
        description="登録されているYouTubeのRSSフィードのURLを表示します",
    )
    async def youtube_listrss(interaction: discord.Interaction):
        embed = discord.Embed(
            title="登録されているYouTube RSSフィードのURLリスト",
    ...
    @bot.tree.command(
        name="youtube-remove-rss",
        description="登録されているYouTubeのRSSフィードのURLを削除します",
    )
    ...
    async def remove_youtube_rss(interaction: discord.Interaction, name: str):
        ...
        if entry_to_remove:
            ...
            await interaction.response.send_message(
                f"RSS URLが削除されました: {name} - {entry_to_remove['url']}"
            )
        else:
            await interaction.response.send_message(
                f"指定された名前のRSSフィードが見つかりませんでした: {name}"
            )
    ...
    async def youtube_now(interaction: discord.Interaction):
        ...
        try:
            # メッセージの送信
            result_message = await bot.youtube_notification.send_message(
                notify_no_update=True
            )
    ...
    @bot.tree.command(
        name="youtube-push", description="Youtubeの通知をGoogleカレンダーに追加します"
    )
    ...
    @bot.tree.command(
        name="youtube-download", description="Youtubeの動画をダウンロードします"
    )
    ...
    @bot.tree.command(
        name="remove-text", description="指定したユーザーのメッセージを削除します"
    )
    async def remove_text(
        interaction: discord.Interaction, user: discord.User, limit: int
    ):
        if limit > 50:
    ...
    @bot.tree.command(
        name="gboard-change", description="gboardの言語をja-JPに変換します"
    )
    async def gboard_change(interaction: discord.Interaction, file: discord.Attachment):
        try:
            ...
            await interaction.response.send_message(
                content="ファイルを変換しました", file=discord.File(zip_file_name)
            )
    ...
    @bot.tree.command(
        name="novel-moneylist", description="登録しているラノベの金額を表示します"
    )
    async def novel_moneylist(interaction: discord.Interaction):
        ...
        try:
            ...
            if not novel_data:
                ...
                embed = discord.Embed(
                    title="ラノベ金額一覧", color=discord.Color.blue()
                )
    ...
    @bot.tree.command(
        name="manga-moneylist", description="登録している漫画の金額を表示します"
    )
    ...
    @bot.tree.command(
        name="bookwalker-salelist",
        description="Googleドライブに保存されているBOOKWALKERのセール情報を表示します",
    )
        ...
        try:
            ...
            if spreadsheets:
                ...
                for item in spreadsheets:
                    item_name_clean = item[
                        "name"
                    ].strip()  # item['name'] の前後の空白を削除
