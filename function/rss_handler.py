...
class RSSHandler:
    ...
    async def send_message(
        self, notify_no_update=False
    ):  # コマンドからの呼び出しの場合は notify_no_update=True
        channel_id = int(os.getenv("RSS_CHANNEL_ID"))  # 通知するチャンネルのID
        ...
        # 最新の記事がなかった場合の通知
        if (
            notify_no_update and not has_new_content
        ):  # コマンドからの呼び出しで、新しい記事がなかった場合
